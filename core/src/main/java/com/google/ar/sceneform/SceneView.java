...
public class SceneView extends SurfaceView implements Choreographer.FrameCallback {
    ...
    public static final String DEFAULT_IBL_LOCATION1 = "environments/studio_small_09_2k.hdr";

    private static final int DEFAULT_MAX_FRAMES_PER_SECONDS = 24;
    ...
    protected void initialize() {
        ...
        if (!AndroidPreconditions.isMinAndroidApiLevel()) {
          ...
          //  scene.getCamera().setVerticalFovDegrees(50);
  ...
  /*   try {
        ...
        try {
            SceneViewKt.setEnvironment(SceneView.this,
                    ResourceLoaderKt.useBuffer(getContext().getAssets().open(DEFAULT_IBL_LOCATION1),
                            (Function1<ByteBuffer, Environment>) buffer ->
                                    HDREnvironmentKt.createEnvironment(HDRLoader.INSTANCE, buffer)));
        } catch (IOException e) {
            e.printStackTrace();
        }
